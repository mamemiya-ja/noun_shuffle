#普通の形態素解析では分割されてしまう名詞も１つの名詞として出す。

def noun_extract(text, wakati):
    #入力テキストを形態素解析
    result = wakati.parse(text).split()
    noun_li = []
    hinshi_li = []
    noun = ""
    for i in range(0, len(result), 2):
        word = result[i]
        #print(word)
        if "と" in word:
            noun_li = []
            hinshi_li = []
            return noun_li, hinshi_li
        
        if word != "EOS":
            try:
                #品詞
                #print(result[i+1])
                morpheme = result[i+1].split(',')[0]
                if morpheme == "名詞" or morpheme == "接頭辞" or morpheme == "接尾辞":
                    noun += word
                elif noun != "":
                    noun_li.append(noun)
                    noun = ""
                    hinshi = result[i-1].split(',')[1]
                    hinshi_li.append(hinshi)
                
            except:
                print("no")
    return noun_li, hinshi_li

def noun_shuffle(text, noun_li, hinshi_li):
    bool = False
    change_element = []
    for i, hinshi in enumerate(hinshi_li):
        tmp = hinshi_li[i+1:]
        tmp_num = i+1
        for hinshi2 in tmp:
            if hinshi == hinshi2:
                word1 = noun_li[i]
                word2 = noun_li[tmp_num]
                change_element.append(word1)
                change_element.append(word2)
                bool = True
                break
            else:
                tmp_num += 1
        if bool == True:
            break
    
    if change_element == []:
        return None
    
    for i, element in enumerate(change_element):
        if i == 0:
            text = text.replace(element, "first_change_part")
        if i == 1:
            text = text.replace(element, "second_change_part")
    
    for i in range(len(change_element)):
        if i == 0:
            text = text.replace("first_change_part", change_element[1])
        elif i == 1:
            text = text.replace("second_change_part", change_element[0])
    
    return text

def noun_shuffle2(text, noun_li, hinshi_li):
    bool = False
    change_element = []
    for i, hinshi in enumerate(hinshi_li):
        tmp = hinshi_li[i+1:]
        tmp_num = i+1
        for hinshi2 in tmp:
            if hinshi == hinshi2:
                word1 = noun_li[i]
                word2 = noun_li[tmp_num]
                word_len = len(word1)
                word_len2 = len(word2)
                pos = text.find(word1)
                pos2 = text.find(word2)
                word_pos = pos + word_len
                word_pos2 = pos2 + word_len2
                if abs(word_pos - pos2) == 1 and (text[pos2-1] == "と" or text[pos2-1] == "や" or text[pos2-1] == "・" or text[pos2-1] == "、" or text[pos2-1] == ","):
                    #print("No")
                    continue
                else:
                    change_element.append(word1)
                    change_element.append(word2)
                    bool = True
                break
            else:
                tmp_num += 1
        if bool == True:
            break
    
    if change_element == []:
        return None
    
    for i, element in enumerate(change_element):
        if i == 0:
            text = text.replace(element, "first_change_part")
        if i == 1:
            text = text.replace(element, "second_change_part")
    
    for i in range(len(change_element)):
        if i == 0:
            text = text.replace("first_change_part", change_element[1])
        elif i == 1:
            text = text.replace("second_change_part", change_element[0])
    
    return text

def hello():
    print("hello")
